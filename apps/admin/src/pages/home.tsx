...
import { CalendarCheck, DoorClosed, Tent as TentIcon, Pizza, DoorOpen, Coins, CircleSlash, CreditCard, FlameKindling, Plus, Info, CircleX, CircleCheck, User, ChevronLeft, ChevronRight, ShoppingBasket, X, FileDown, ReceiptText, CheckIcon } from "lucide-react"
import { NotificationDto, PublicExperience, PublicProduct, Reserve, ReserveExperienceDto, ReserveExtraItemDto, ReserveTentDto, createReserveExperienceDto, createReserveProductDto } from "../lib/interfaces";
...
import { addExperienceToReserve, addProductToReserve, addExtraItemToReserve, getPublicExperiences, getPublicProducts } from "../db/actions/reserves";
import ExperienceCard from "../components/ExperienceCard";
import { toast } from "sonner";
import ProductCard from "../components/ProductCard";
import { useNavigate } from "react-router-dom";
import { NO_NOTIFICATIONS, NO_RESERVATIONS } from "../assets/images";
...
const ReserveCard = (props: ReserveCardProps) => {
  const { fetchReserves, currentPage, reserve } = props;
  const { user } = useAuth();
  ...
  const [loadingCreateExtraItems, setLoadingCreateExtraItems] = useState(false);

  const [experienceAddQuery, setExperienceAddQuery] = useState("");
  const [productAddQuery, setProductAddQuery] = useState("");
  ...
  const [experiencesDB, setExperiencesDB] = useState<PublicExperience[]>([]);
  const [experiences, setExperiences] = useState<createReserveExperienceDto[]>([]);

  const [productsDB, setProductsDB] = useState<PublicProduct[]>([]);
  const [products, setProducts] = useState<createReserveProductDto[]>([]);

  const filteredExperiencesDB = useMemo(() => {
    const q = experienceAddQuery.trim().toLowerCase();
    if (!q) return experiencesDB;
    return experiencesDB.filter(e =>
      (e.name || "").toLowerCase().includes(q)
    );
  }, [experienceAddQuery, experiencesDB]);

  const filteredProductsDB = useMemo(() => {
    const q = productAddQuery.trim().toLowerCase();
    if (!q) return productsDB;
    return productsDB.filter(p =>
      (p.name || "").toLowerCase().includes(q)
    );
  }, [productAddQuery, productsDB]);
  ...
  const HandlerConfirmEntity = async (entityType: string, idReserve: number, entityId?: number) => {
    ...
    if (user !== null) {
      const confimedEntity = await confirmEntity(entityType, idReserve, user.token, i18n.language, entityId);
      if (confimedEntity) {
        fetchReserves(currentPage);
      }
    }
      ...
      <Modal isOpen={openReserve} onClose={() => setOpenReserve(false)}>
        <div className="w-screen lg:w-[800px] h-auto lg:h-[600px] flex flex-col items-start justify-start text-secondary py-16 px-4 sm:p-6 overflow-hidden">
          <div className="w-full h-auto flex flex-row gap-x-4 sm:gap-x-6 pb-4 border-b-2 border-secondary">
            ...
            <InputRadio
              className="w-auto"
              onClick={() => { setOpenDetails("extraItems"); setSelectedOption(0); setStateAdd(undefined) }}
              name="category"
              ...
              <motion.div
                ...
                {reserve.products.length > 0 ? (
                  ...
                  <div className="w-full h-[200px] flex justify-center items-center flex-col">
                    ...
                    <p className="text-secondary text-sm">{t("reserve.no_products_available")}</p>
                    <Button
                      onClick={() => setStateAdd("add_product")}
                      className="w-auto mt-4"
                      effect="default"
                ...
                className="w-full flex flex-col-reverse sm:flex-row py-4 items-start max-sm:gap-y-4 sm:items-center justify-start">
                {
                  reserve.experiences.length > 0 ? (
                    ...
                    <div className="w-full h-[200px] flex justify-center items-center flex-col">
                      ...
                      <p className="text-secondary text-sm">{t("reserve.no_experiences_available")}</p>
                      <Button
                        onClick={() => (setStateAdd("add_experience"), setSelectedExperience(undefined))}
              ...
              <motion.div
                ...
                <div className="w-full lg:w-[50%] h-full flex flex-col justify-start items-start overlfow-hidden border-2 border-slate-200 rounded-md p-2">
                  <div className="w-full h-full flex flex-col justify-start items-start overlfow-y-scroll">
                    ...
                    <input
                      value={experienceAddQuery}
                      onChange={(e) => setExperienceAddQuery(e.target.value)}
                      placeholder={t("common.search")}
                      className="w-full border border-slate-300 rounded-md px-3 py-2 text-sm outline-none focus:ring-2 focus:ring-secondary mb-3"
                    />
                    <div className="w-full h-auto flex flex-col gap-y-3 overflow-y-scroll pr-2">
                      {filteredExperiencesDB.map((experienceItem, index) => {
                        return (
              ...
              <motion.div
                ...
                <div className="w-full lg:w-[50%] h-full flex flex-col justify-start items-start overlfow-hidden border-2 border-slate-200 rounded-md p-2">
                  <div className="w-full h-full flex flex-col justify-start items-start overlfow-y-scroll">
                    ...
                    <input
                      value={productAddQuery}
                      onChange={(e) => setProductAddQuery(e.target.value)}
                      placeholder={t("common.search")}
                      className="w-full border border-slate-300 rounded-md px-3 py-2 text-sm outline-none focus:ring-2 focus:ring-secondary mb-3"
                    />
                    <div className="w-full h-auto flex flex-col gap-y-3 overflow-y-scroll pr-2">
                      {filteredProductsDB.map((productItem, index) => {
                        return (
            ...
            <motion.div initial="hidden" animate="show" exit="hidden" variants={fadeIn("left", "", 0.5, 0.5)} className="w-full flex flex-col gap-y-6 py-4">
              {reserve.extraItems && reserve.extraItems.length > 0 ? (
                <>
                  <div className="w-full h-auto flex flex-row justify-end items-center">
                    <Button
                      onClick={() => setStateAdd("add_extraItem")}
                      className="w-auto"
                      effect="default"
                      size="sm"
                      variant="ghostLight"
                      rightIcon={<Plus />}
                    >{t("reserve.add_extra_item")}</Button>
                  </div>
                  <div className="w-full h-full sm:h-[300px] flex flex-col">
          ...
          {stateAdd === "add_extraItem" && openDetails === "extraItems" && (
            <>
              <div className="w-full h-[10%] flex flex-row justify-end items-center mt-2">
                <Button
                  onClick={() => setStateAdd(undefined)}
                  ...
                  variant="ghostLight"
                  rightIcon={<ShoppingBasket />}
                >{t("reserve.my_extra_items")}</Button>
              </div>

              <motion.div initial="hidden" animate="show" exit="hidden" variants={fadeIn("up", "", 0.5, 0.5)}
                className="w-full h-[90%] flex flex-col lg:flex-row py-4 gap-y-4 lg:gap-x-2 pb-12">

                {/* Cart */}
                <div className="w-full lg:w-[50%] h-full flex flex-col border-2 border-slate-200 rounded-md p-2">
                  <span className="flex flex-row items-end gap-x-2">
                    <ShoppingBasket className="h-5 w-5" />
                    <h2 className="text-lg">{t("reserve.list")}</h2>
                  </span>
                  <div className="w-full h-full flex flex-col gap-y-3 mt-4 overflow-y-scroll">
                    {extraItems.length > 0 ? extraItems.map((it, idx) => (
                      <div key={`extra_cart_${idx}`} className="flex flex-row w-full h-auto border-2 border-slate-200 rounded-lg shadow-sm p-4">
                        <div className="flex flex-col h-full w-auto">
                          <span className="text-sm mt-auto text-tertiary">{it.name}</span>
                          <div className="flex flex-row gap-x-2">
                            <span className="text-xs text-secondary">{t("reserve.quantity")} :</span>
                            <span className="text-xs mt-auto">{it.quantity}</span>
                          </div>
                          <div className="flex flex-row gap-x-2">
                            <span className="text-xs text-secondary">{t("reserve.unit_price")}:</span>
                            <span className="text-xs mt-auto">{formatPrice(it.price)}</span>
                          </div>
                        </div>
                        <div className="flex flex-col items-end h-full w-[20%] ml-auto">
                          <span onClick={() => setExtraItems(prev => prev.filter((_, i) => i !== idx))}
                            className="h-5 w-5 mb-auto active:scale-95 hover:scale-110 duration-300 cursor-pointer hover:text-tertiary">
                            <X className="w-full h-full" />
                          </span>
                          <span className="mt-auto">{formatPrice((it.quantity ?? 0) * it.price)}</span>
                        </div>
                      </div>
                    )) : <p className="text-xs mt-2">{t("reserve.no_extra_items_to_add")}</p>}
                  </div>
                  <div className="w-full h-[10%] flex justify-end">
                    <Button
                      onClick={async () => {
                        ...
                        setLoadingCreateExtraItems(false);
                      }}
                      isLoading={loadingCreateExtraItems}
                      className="w-auto mt-auto ml-auto"
                      effect="default"
                      size="sm"
                      variant="ghostLight"
                      rightIcon={<Plus />}
                    >{t("reserve.request_adding_extra_items")}</Button>
                  </div>
                </div>

                {/* Simple form (name/price/qty) */}
                <div className="w-full lg:w-[50%] h-full flex flex-col border-2 border-slate-200 rounded-md p-3 gap-3">
                  <h2 className="text-lg">{t("reserve.add_extra_item")}</h2>
                  <input
                    value={extraName}
                    onChange={(e) => setExtraName(e.target.value)}
                    placeholder={t("reserve.extra_item_name")}
                    className="border border-slate-300 rounded-md px-3 py-2 text-sm outline-none focus:ring-2 focus:ring-secondary"
                  />
                  <input
                    type="number"
                    value={extraPrice}
                  ...
                  <input
                    ...
                    placeholder={t("reserve.quantity")}
                    className="border border-slate-300 rounded-md px-3 py-2 text-sm outline-none focus:ring-2 focus:ring-secondary"
                  />
                  <Button
                    onClick={() => {
                      if (!extraName || !extraPrice || !extraQty) { toast.error(t("common.fill_required")); return; }
                      setExtraItems(prev => [...prev, {
                        ...
                        confirmed: false
                      } as ReserveExtraItemDto]);
                      setExtraName(""); setExtraPrice(""); setExtraQty("");
                    }}
                    className="w-auto"
                    effect="default"
                    size="sm"
                ...
                <div className="h-full w-[50%] lg:w-[75%] flex flex-col pb-4 px-4">
                  ...
                  <div className="w-full h-auto flex flex-col lg:flex-row mt-2">
                    <div className="w-[100%] lg:w-[50%] h-full flex flex-col">
                      ...
                      <div className="w-full h-auto flex flex-col lg:flex-row gap-x-6 mt-4">
                        <p className="text-primary text-xs">{t("experience.duration")}:</p>
                        <p className="text-gray-400 text-sm">
            ...
            <div className="w-full h-full overflow-hidden">
              {dataNotifications.notifications.length === 0 ?
                <div className="w-full h-[400px] sm:h-full flex flex-col items-center justify-center bg-white gap-y-4">
                  <img src={NO_NOTIFICATIONS} alt="tent" className="w-[40%] sm:w-[100px] h-auto object-cover" />
                  <h3
        ...
        <div className="bg-white px-2 py-4  xl:p-4 rounded-lg shadow-lg border-2 border-gray-200 w-full xl:w-[50%] h-auto xl:h-full flex flex-col ">
          ...
          <div className="w-full h-[80%] flex flex-col overflow-y-scroll xl:pr-4 gap-y-4 mt-4">
            {datasetReserves.reserves.length === 0 ?
              <div className="w-full h-[400px] sm:h-full flex flex-col items-center justify-center bg-white gap-y-4">
                <img src={NO_RESERVATIONS} alt="tent" className="w-[40%] sm:w-[30%] h-auto object-cover ml-6" />
                <h3
                  className={`text-secondary text-center`}>
                  {`${t("reserve.no_tents_reservations")}`}
                </h3>
                <Button
                  onClick={() => goToRoute("/reserves")}
                  effect="default"
                  variant="ghostLight"
                  size="lg"
            ...
            <div className="w-full h-auto xl:h-full flex flex-col overflow-y-scroll gap-y-4 mt-4">
              ...
              {dataNotifications.notifications.length === 0 ?
                <div className="w-full h-auto flex flex-col items-center justify-center bg-white gap-y-4 py-12">
                  <img src={NO_NOTIFICATIONS} alt="tent" className="w-[150px] sm:w-[200px] h-auto object-cover" />
                  <h3
                    className={`text-secondary text-center`}>
                    {`${t("reserve.no_notifications")}`}
                  </h3>
