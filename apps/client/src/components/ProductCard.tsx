...
const ProductCard: React.FC<propsItemExtra> = (props: propsItemExtra) => {
  ...
  const handleAdd = () => {
    handleAddProduct(product.id, quantity);
    setQuantity(0);
  }
  ...
  return (
    <motion.div
      ...
      <div className="w-full h-[25%] flex flex-col justify-start items-start py-2 px-4">
        ...
        <div className={`w-full h-auto p-none m-none flex ${variant == "line" ? "flex-row" : "flex-col"}`}>
          ...
          <div className={`${variant == "line" ? "w-[50%]" : "hidden"}  h-[50px] flex flex-row justify-end ps-4 py-2`}>
            <button onClick={() => handleAdd()} className="p-2 bg-secondary text-white rounded-full group hover:bg-white hover:border-2 hover:border-secondary hover:text-secondary h-full w-auto flex items-center justify-center active:scale-95 duration-300 text-xs"><CalendarPlus className="h-full w-full" /></button>
          </div>
      ...
      <div className={`${variant == "line" ? "hidden" : "w-full"} h-[15%] flex flex-row justify-end px-4 py-2`}>
        <button onClick={() => handleAdd()} className="px-2 bg-secondary text-white rounded-full group hover:bg-white hover:border-2 hover:border-secondary hover:text-secondary h-full w-auto flex items-center justify-center active:scale-95 duration-300 text-xs">{t("reserve.add_to_reserve")}</button>
      </div>
    </motion.div>
  )
}
